...
const fileToBase64 = <T extends { file: File; base64: string }>(file: File): Promise<T> => {
...
const PromptForm: React.FC<PromptFormProps> = ({ onGenerate, initialValues }) => {
        ...
        <div className="mb-3 p-4 bg-[#2c2c2e] rounded-xl border border-gray-700 flex flex-col items-center justify-center gap-4">
          ...
          {startFrame && !endFrame && (
            <div className="mt-3 flex items-center">
              ...
              />
              <label htmlFor="loop-video-checkbox" className="ml-2 text-sm font-medium text-gray-300 cursor-pointer">
        ...
        <div className="mb-3 p-4 bg-[#2c2c2e] rounded-xl border border-gray-700 flex flex-wrap items-center justify-center gap-2">
          {referenceImages.map((img, index) => (
            <ImageUpload
              ...
              label=""
              onSelect={() => {}}
              onRemove={() => setReferenceImages((imgs) => imgs.filter((_, i) => i !== index))}
            />
          ))}
  ...
  switch (generationMode) {
    ...
    case GenerationMode.EXTEND_VIDEO:
      ...
      if (isSubmitDisabled) tooltipText = 'An input video from a previous generation is required to extend.';
      break;
  }
        ...
        <div className="flex items-end gap-2 bg-[#1f1f1f] border border-gray-600 rounded-2xl p-2 shadow-lg focus-within:ring-2 focus-within:ring-indigo-500">
          <div className="relative" ref={modeSelectorRef}>
            <button
              ...
              <span className="font-medium text-sm whitespace-nowrap">{generationMode}</span>
            </button>
            {isModeSelectorOpen && (
